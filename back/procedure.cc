...
static std::queue<super_block*> *branchBlockQueue = NULL;
...
std::vector<super_block_path*> * findBlockPath(super_block *begin, super_block *end){
	...
	assert(begin != end);
	if(branchBlockQueue == NULL){
		branchBlockQueue = new std::queue<super_block*>();
	}
...
std::vector<super_block_path*> * Procedure::findSubPathInDoms(super_block *dom_block_begin, super_block* dom_block_end){
	...
	/**
	 * The following code are used to find and test the branch block which are used to find the permitted spawn range.
	 */
...
/**
 * Find All the ProceurePath of this procedure (from the entry block to the exit block). These ProcedurePath will be used in the future thread partition.
 */
void Procedure::findAllProcedurePathList(std::ofstream& os){
	super_block *start_block = the_scfg->entry();
	super_block *end_block = the_scfg->exit();
	...
	if(sameCount == 0){
		...
		assert(0);					//如果代码没有bug, 则， 之前寻找的路径中必然包含最可能路径， 所以这里用 assert(0)断言来验证这样一种推论。
	}

//	// 接下来的这段代码用来打印经过删除重复的最可能路径之后的打印结果。
//	for(int i = 0; i < pathList->size(); i++){
...
/**
 * Find all the dominators of this procedure. This means that, find the super blocks that are the dominators of the entry of the procedure.
 * @out, 结果存放在属性dominatorBlockList上。
 */
...
void Procedure::processProcedure(){
	 ...
	 */
	static std::ofstream theDataStream("testcase/holiday.txt");
...
#define CATCH_DATA_
#ifdef CATCH_DATA_
		theDataStream << "*****The most likely path is *****:" ;
		this->likelyPath->printPath(theDataStream);
		theDataStream << "###########Proc: " << cur_psym->name() <<"该过程共有贯穿过程的路径#########:" << pathList->size()  << "条"<< "\n";
		for(int i = 0; i < pathList->size(); i++){
			pathList->at(i)->printPath(theDataStream);
		}
		this->printCqipList(theDataStream);
		theDataStream << std::endl << std::endl << std::endl;
#endif
		std::cout << "*****The most likely path is *****:" ;
		this->likelyPath->printPath(std::cout);
		std::cout << "###########Proc: " << cur_psym->name() <<"该过程共有贯穿过程的路径#########:" << pathList->size()  << "条"<< std::endl;
		for(int i = 0; i < pathList->size(); i++){
			pathList->at(i)->printPath(std::cout);
		}
...
void Procedure::findPermitBranchBlock(){
